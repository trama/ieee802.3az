#include "internalHub_EEE_intel.h"
#include "EPLframe_m.h"
#include <cassert>
#include "powerListener.h"

Define_Module(internalHub_EEE_intel);

void internalHub_EEE_intel::initialize(int stage) {

    if (stage == 0){
        hasSubnetworks = par("hasSubnetworks");
        EV << "I am the relay unit for EEE. Init... "<<endl;
    }
...
void internalHub_EEE_intel::handleAndDispatchFrame( EtherFrame *frame, int inputport ) {
...
void internalHub_EEE_intel::broadcastFrame( EtherFrame *frame, int inputport ) {
    for ( int i = 0 ; i < numPorts ; ++i )
        if ( i != inputport )
...
void internalHub_EEE_intel::printAddressTable() {
    ...
    for ( iter = addresstable.begin(); iter != addresstable.end() ; ++iter ) {
        EV<< "  " << iter->first << " --> port" << iter->second.portno << endl;
    }
}

int internalHub_EEE_intel::getPortForAddress( MACAddress& address ) {
...
int internalHub_EEE_intel::isMulticast(MACAddress& addr){
...
void internalHub_EEE_intel::readAddressTable(const char* fileName){
