#include "switchEEE_100baseTX.h"
#include "EPLframe_m.h"
#include <cassert>
#include "powerListener.h"

Define_Module(switchEEE_100baseTX);

void switchEEE_100baseTX::initialize() {
    ...
    EV<< "powerSignal ID " << powerSignal << endl;
    rcvdPwNoEEESignal = registerSignal("rcvdPwNoEEE");
    EV<< "rcvdPwNoEEESignal ID " << rcvdPwNoEEESignal << endl;
    // TODO to be deleted?
    pkSignal = registerSignal("pk");
    EV<< "pkSignal ID " << pkSignal << endl;

    // creation of listeners for power consumption signals
    ...
    EV<< "noLpi_powerSignal ID " << noLpi_powerSignal << endl;
...
void switchEEE_100baseTX::handleSelfMsg(cMessage *msg) {
    switch (msg->getKind()) {
        case END_IFG:
...
void switchEEE_100baseTX::handleLowerMsg( cMessage *msg ) {
...
void switchEEE_100baseTX::processFrame(cMessage * msg){
    numMessages++;
    emit(pkSignal, msg);
...
void switchEEE_100baseTX::manageLpiSignals(cMessage *msg){
    int arrivalPort = msg->getArrivalGate()->getIndex();
...
void switchEEE_100baseTX::goLPI(int portNumber) { // Switch to LPI mode
...
void switchEEE_100baseTX::wakeUp(int portNumber) {
...
void switchEEE_100baseTX::finish() {
    simtime_t t = simTime();
...
void switchEEE_100baseTX::handleMessage(cMessage *msg) {
...
void switchEEE_100baseTX::checkConnections(bool errorWhenAsymmetric) {
...
void switchEEE_100baseTX::handleAndDispatchFrame( EtherFrame *frame, int inputport ) {
...
void switchEEE_100baseTX::broadcastFrame( EtherFrame *frame, int inputport ) {
    for ( int i = 0 ; i < numPorts ; ++i )
        if ( i != inputport ) send((EtherFrame*) frame->dup(), "ethg$o", i);
    delete frame;
}

void switchEEE_100baseTX::printAddressTable() {
...
int switchEEE_100baseTX::getPortForAddress( MACAddress& address ) {
...
int switchEEE_100baseTX::isMulticast(MACAddress& addr){
...
void switchEEE_100baseTX::readAddressTable(const char* fileName){
