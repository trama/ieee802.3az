#include "relayUnitForEEE.h"
#include "EPLframe_m.h"
#include <cassert>
#include "powerListener.h"

Define_Module(relayUnitForEEE);

void relayUnitForEEE::initialize(int stage) {
...
void relayUnitForEEE::handleAndDispatchFrame( EtherFrame *frame, int inputport ) {
...
void relayUnitForEEE::broadcastFrame( EtherFrame *frame, int inputport ) {
    for ( int i = 0 ; i < numPorts ; ++i )
        if ( i != inputport )
...
void relayUnitForEEE::printAddressTable() {
...
int relayUnitForEEE::getPortForAddress( MACAddress& address ) {
...
int relayUnitForEEE::isMulticast(MACAddress& addr){
...
void relayUnitForEEE::readAddressTable(const char* fileName){
